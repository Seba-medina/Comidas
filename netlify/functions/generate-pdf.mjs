...
export default async (req) => {
  ...
  for (let i = 0; i < diffDias; i++) {
    ...
    const titulo = `${DIAS_SEMANA[fecha.getUTCDay()]} ${fecha.getUTCDate()} de ${MESES[fecha.getUTCMonth()]} de ${fecha.getUTCFullYear()}`;
  ...
  return new Response(pdfBytes, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="comidas_${desde}_a_${hasta}.pdf"`,
    },
  });
};
